...
void Region::get_reads(char** bam_files, int num_bam_files, int intron_len_filter, int filter_mismatch, int exon_len_filter)
{
	...
	* **************/
	...
	for (int i=0; i<all_reads.size(); i++)
	{
	    ...
	    if (all_reads[i]->max_intron_len()<intron_len_filter && all_reads[i]->min_exon_len()>exon_len_filter && all_reads[i]->get_mismatches()<=filter_mismatch && all_reads[i]->multiple_alignment_index==1)
        {
	        reads.push_back(all_reads[i]);
        }

	    /*
        fprintf(fd_out, "## %d : %d\n", (int) all_reads[i]->max_intron_len(), intron_len_filter);
	    fprintf(fd_out, "## %d : %d\n", (int) all_reads[i]->min_exon_len(), exon_len_filter);
	    fprintf(fd_out, "## %d : %d\n", (int) all_reads[i]->get_mismatches(), filter_mismatch);
	    fprintf(fd_out, "## %d : 0\n", (int) all_reads[i]->multiple_alignment_index);
        break;
        */
	}
